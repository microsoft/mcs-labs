...
### Fixed
- **`core-concepts-agent-knowledge-tools` UC#1 — require the previous experience (New experience default-on breaks creation).** Copilot Studio's default-on **New experience** replaces the Home "What would you like to build?" describe textbox with **Agent / Workflow** cards and creates a **new-type agent that has no Topics** (used in UC#4) and no auto-generated instructions, so UC#1–UC#4 no longer match. Added an IMPORTANT note at UC#1 instructing learners to turn the **New experience** toggle off ("Switch back") before creating the agent, with the toggle tradeoff spelled out (the previous experience omits the newest agent capabilities, including the new orchestrator; this Topics-based lab requires the classic flow).
