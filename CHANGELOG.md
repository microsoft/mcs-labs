...
### Added
- **Backlog Tracker at `/tracker/`.** New static page surfacing GitHub issues across 5 categories (Bugs, New Labs, Content Updates, Portal Enhancements, Bootcamp Features). Two tabs: **Insights** (default — KPI tiles, donut by category, opened-vs-closed trend line with date/count axes, per-category aging breakdown with stacked bucket bars + named oldest issues, top-15 oldest open issues table) and **Board** (Kanban with Triage / Backlog / In Progress / Done columns, filter chips per category, smart "+ New item" button that picks the right GitHub issue template based on the active filter). 100% public — any deeper interaction (assignees, comments, history) deep-links to the corresponding GitHub issue. Data refreshes via a 15-minute GitHub Actions cron (`.github/workflows/tracker-data.yml`) that commits a normalized snapshot to `assets/data/issues.json` only when content has changed (idempotent diff against the prior committed snapshot, ignoring `generatedAt`). A manual **Refresh** button next to the snapshot timestamp re-fetches the JSON on demand and reports whether the file actually changed ("✓ New data loaded" vs. "✓ No changes — snapshot still current"). Time-range selector (7d / 14d / 30d / 60d / 90d) drives the Closed KPI, trend chart x-axis, and Done column window. Linked from the home-page footer only — not site-wide nav. Inherits `layout: single` so masthead, footer, and theme switcher are consistent with the rest of the site. Lab Assistant + "Request an account" CTA are suppressed on this page (status dashboard, not a help context). Module split: `tracker.md` (page shell), `_layouts/single` (inherited), `assets/css/tracker.css`, `assets/js/tracker/{data,insights,board,main}.js`, `_data/tracker.yml` (repo config + stale threshold), `scripts/build-tracker-data.js` (GraphQL fetch + shape, with Node 20 built-in test runner unit tests).
