...
### Fixed
- **`core-concepts-agent-knowledge-tools` UC#1 — create in the previous experience, then switch back to the new experience (New experience default-on breaks creation).** Copilot Studio's default-on **New experience** replaces the Home "What would you like to build?" describe textbox with **Agent / Workflow** cards and creates a **new-type agent that has no Topics** (used in UC#4) and no auto-generated instructions, so the creation flow no longer matches. Added an IMPORTANT note at UC#1 walking learners through the round-trip: turn the **New experience** toggle **off** ("Switch back") to showcase that you can return to the previous experience and to use the describe-driven creation, create the agent, then **turn the New experience back on** and do all remaining edits (knowledge, tools, topics, testing) in the new UX — the classic agent reopens with the same Overview/Knowledge/Tools/Topics tabs there. Tradeoff noted (the previous experience omits the newest agent capabilities, including the new orchestrator).
