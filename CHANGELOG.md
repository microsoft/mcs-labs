...
### Fixed
- **`mcs-orchestration` UC#3 — "+ Create blank agent" replaced by "New Agent ▾ → New classic agent" (New experience).** Copilot Studio's default-on **New experience** removed the standalone *+ Create blank agent* button from the Agents page; the classic path is now the **New Agent** split-button dropdown → **New classic agent**. UC#3 step 2 wording updated, `labs/mcs-orchestration/images/image-43.png` refreshed to the new Agents page, and a New-experience toggle-tradeoff note added (turning the toggle off loses the new-agent capabilities, including the new orchestrator).
