...
### Fixed
- **`mcs-orchestration` UC#1 — added a "Load the Dataverse sample data" step.** A brand-new environment doesn't ship the standard `(sample)` Account/Contact records (Fourth Coffee, Litware, Adventure Works, …) the lab relies on, so UC#1 now begins by installing them via **Power Platform admin center → Environment → Settings → Sample Data → Install Sample Data** (with a skip note if already installed). Adds `labs/mcs-orchestration/images/uc1-sample-data-install.png`.
