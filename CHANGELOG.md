...
### Fixed
- **`mcs-github-harness` screenshots are now cropped to their subject.** All 14 walkthrough images were full-viewport captures — 1263 px wide including the left navigation, page header and unrelated chrome — which left the actual subject a small region in the middle. Every image is re-captured tight: agent responses to the response block, the component rail to the rail, the SharePoint and skill dialogs to the dialog. Widths now run 400-1216 px according to what each image is showing.
