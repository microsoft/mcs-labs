...
### Changed
- **Bootcamp module renumbering — no gaps in the agenda.** The Day 3 sequence used to jump Module 9 → 11 → 13 → 14 → 15 → 16 (Modules 10 and 12 were dropped earlier). Renumbered to a contiguous 1–14 sequence: Module 11 (Governance) → 10, Module 13 (Multi-Agent) → 11, Module 14 (Autonomous Agents) → 12, Module 15 (Skills for Copilot Studio) → 13, Module 16 (Agent 365) → 14. Updated session titles + slides paths in `_data/agendas/bootcamp.yml`, renamed the five deck files in `presentations/bootcamp/` to match (e.g. `13. Multi Agent_CB.pptx` → `11. Multi Agent_CB.pptx`), and rewrote the Day 3 schedule table on the Bootcamp Host Deck to reflect the new numbers.
