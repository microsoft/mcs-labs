...
- **14 scaffolded module entities for V3** (`_modules/`): `agents-workflows-making-them-real`, `cowork`, `harnesses-and-component-models`, `copilot-harness`, `standard-harness`, `github-copilot-harness`, `creation-github-harness`, `consumption-management`, `knowledge-and-tools-deep-dive`, `instructions-and-descriptions`, `agent-skills`, `coding-agents`, `alm-and-analytics`, `governance-and-agent-365`. Each carries `placeholder: true` and a note naming the source material it will be assembled from, following the existing `workflows` placeholder convention. V3 reuses three existing entities unchanged — `agent-builder` (M2), `workflows` (M9), and `multi-agent` (M14). Labs 5, 8 and 9 do not exist yet and are carried as `session` rows so the timeline stays complete and correctly timed; each names its seed use cases and converts to a `type: lab` row as the lab lands.
- **V3 slide decks are now in-repo source** (`presentations/bootcamp-v3/`, Git LFS — 16 decks, ~160 MB). Sixteen of V3's 17 modules link to a deck (M10 *Consumption Management* is net-new and has none yet), so the agenda and the module pages both render a **Slides** / **Download Slides** button, matching how `presentations/bootcamp/` backs the V2.1 event. Thirteen of the 14 scaffolded module entities carry `slides:` in front matter; the three reused entities (`agent-builder`, `workflows`, `multi-agent`) get their V3 deck via a per-row `slides:` override in `bootcamp-v3.yml`, so the V2.1 event keeps pointing at its own decks.
...
- **`description:` is now overridable per module row on an event agenda**, joining the existing `title:`, `duration:` and `slides:` overrides. Lets an event reuse a module entity whose own description doesn't fit that event's framing — V3's Module 2 reuses `agent-builder` but describes Microsoft 365 Copilot, since Agent Builder moves to Module 5.
