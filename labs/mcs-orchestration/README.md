...
- The **Order Management MCP** and **Warehouse MCP** custom MCP servers imported into your environment — see [Custom MCP servers](#custom-mcp-servers) below. **Use Case #4 cannot be completed without them.**

#### Custom MCP servers

Use Case #4 attaches two custom MCP servers to the agent: **Order Management MCP** and **Warehouse MCP**. Neither is part of the platform — both are custom connectors that have to already exist in your environment, and nothing inside this lab creates them. If they are missing you will not find out here; you will find out in Use Case #4, when the tools you are told to add are not in the picker.
...
> [!NOTE]
> The **Microsoft Dataverse MCP Server**, also attached in Use Case #4, is first-party and needs no import — it appears in the **Model Context Protocol** tool list on its own. Only these two custom servers have to be provisioned.
